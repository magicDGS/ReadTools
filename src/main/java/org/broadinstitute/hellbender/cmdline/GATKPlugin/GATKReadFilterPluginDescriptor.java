 ...
 */
// TODO: from https://github.com/broadinstitute/gatk/pull/2401 (latest commit 01-03-2017)
public class GATKReadFilterPluginDescriptor extends CommandLinePluginDescriptor<ReadFilter> {
    ...
    @Argument(fullName = StandardArgumentDefinitions.READ_FILTER_LONG_NAME,
            ...
            doc="Read filters to be applied after analysis", optional=true, common = true)
    ...
    @Argument(fullName = StandardArgumentDefinitions.DISABLE_READ_FILTER_LONG_NAME,
            ...
            doc="Read filters to be disabled after analysis", optional=true, common = true)
    ...
    public List<ReadFilter> getAllInstances() {
        // TODO: this is a change w.r.t. the GATK PR (comment at the top of the class)
        // TODO: we require that the disable ones are not included
        // TODO: and that enabled disabled defaults return the instance here
        // TODO: this will change in the future in GATK for the new Barclay interface (I hope):
        ...
        userEnabledReadFilterNames.forEach(s -> {
            // if the user did not disable it
            if (!userDisabledReadFilterNames.contains(s)) {
                if (!toolDefaultReadFilters.containsKey(s)) {
                    filters.add(allDiscoveredReadFilters.get(s));
    ...
    public List<Object> getDefaultInstances() {
        // TODO: this is a change w.r.t. the GATK PR (comment at the top of the class)
        // TODO: we require that the disable ones are not included
        // TODO: and that disabling all will not return any instance
        // TODO: this will change in the future in GATK for the new Barclay interface:
        // TODO: https://github.com/broadinstitute/barclay/pull/38
        return (disableToolDefaultReadFilters)
                ? new ArrayList<>()
                : toolDefaultReadFilters.entrySet()
                        .stream()
                        .filter(s -> !isDisabledFilter(s.getKey()))
                        .map(s -> s.getValue())
